...
int delay = 1000;
int tick = 0;  // tick and tock used for debouncing
int tock = 0;
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  sprintf(buffer, "%d \r\n", adc_val);
	  HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 1000);     // adc value

	  //TASK 3
	  //Test your ADCtoCRR function. Display CRR value via UART

	  sprintf(buffer, "%d\r\n", ADCtoCRR(adc_val));
	  HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 1000);     // duty cycle value

	  //TASK 4
	  //Complete rest of implementation

	  __HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_4, ADCtoCRR(adc_val));  // green led brightness
...
void EXTI0_1_IRQHandler(void)
{
	...
	tick = HAL_GetTick();       // get current system time
	if (tick - tock > 10)       // if difference from last recorded time (tock) is > 10 ms
	{
		tock = tick;            // set last recorded time to current time
		if (delay == 1000)
			{
				delay = 500;
			}
...
uint32_t pollADC(void){
	...
	HAL_ADC_Start(&hadc);                    // start ADC conversion using ADC_HandleTypeDef
	uint32_t val = HAL_ADC_GetValue(&hadc);  // get ADC value and as uint32_t
...
uint32_t ADCtoCRR(uint32_t adc_val){
	...
	int CRR = (adc_val * 47999) / 4095;       // convert adc_val to appropriate duty cycle scale
											  // crr = adc * (47999/4095)
