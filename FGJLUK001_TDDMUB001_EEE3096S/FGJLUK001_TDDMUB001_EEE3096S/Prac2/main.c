...
int main(void){
  ...
  //TASK 6
  //YOUR CODE HERE
  //setTime(0, 0, 0, 2, 27, 9, 22); //  set time to start of today
  ...
  while (1)
  {
	...
	//HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 10000000);

	//YOUR CODE HERE
	getTime();

	sprintf(buffer, "Date: %02d-%02d-20%02d \r\n", time.dayofmonth, time.month, time.year); // print Date: dd-mm-yyyy
	HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 10000);

	sprintf(buffer, "Time: %02d:%02d:%02d \r\n", time.hour, time.minutes, time.seconds); // print Time: hh:mm:ss
	HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 10000);

	sprintf(buffer, "%d \r\n", epochFromTime(time));
	HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 10000); // print epoch time

	pause_sec(1.00); // manual delay
	HAL_Delay(1000); // accurate delay in ms
...
void pause_sec(float x)
{
	...
	for (int i = 0; i < x; ++i)
	{
		...
		{
			// do nothing for DELAY1*DELAY2 increments
		}
	}
}
...
uint8_t decToBcd(int val)
{
	...
	//YOUR CODE HERE
	uint8_t tens = (val/10) << 4; // integer divide to get 10s digit as uint8_t, move to first 4 bits
	uint8_t units = val % 10;     // mod to get units digit as last 4 bits
	uint8_t bcd = tens + units;   // add to get number in bcd
	return bcd;
...
int bcdToDec(uint8_t val)
{
	...
	//YOUR CODE HERE
	int tens = ((val & 0b11110000) >> 4)*10;  // binary AND to get only first 4 bits, move to units as int and * 10
	int units = val & 0b00001111;             // binary AND to get only last 4 bits as an int value
	int dec = tens + units;                   // add to get number in dec
	return dec;
...
void setTime (uint8_t sec, uint8_t min, uint8_t hour, uint8_t dow, uint8_t dom, uint8_t month, uint8_t year)
{
	...
	//YOUR CODE HERE
	set_time[0] = decToBcd(sec);    // assign params to set_time array
	...
	HAL_I2C_Mem_Write(&hi2c1, DS3231_ADDRESS, 0x00, 1, set_time, 7, 1000);    // write set_time array to rtc
...
void getTime (void)
{
	...
	HAL_I2C_Mem_Read(&hi2c1, DS3231_ADDRESS, 0x00, 1, get_time, 7, 1000);  // read get_time array from rtc

	time.seconds = bcdToDec(get_time[0]);  // update time values from get_time array values
...
int epochFromTime(TIME time){
	...
	int years = time.year - 22;      // number of years from 2022
	int months = time.month;
	int day = time.dayofmonth - 1;   // day - 1 (not counting the 24 hours of the current day)
	int hours = time.hour;
	...
	switch(months){    // convert month to equivalent days
	case 1:
		day += 0;      // Still in January
	break;
	case 2:
		day += 31;     // Add all of January
	break;
	case 3:
		day += 59;      // Add all of Jan + Feb
	break;
	case 4:
		day += 90;      // Add all of Jan + Feb + March
	break;
	case 5:
		day += 120;      // Add all of Jan + Feb + March + April
	break;
	case 6:
		day += 151;      // Add all of Jan + Feb + March + April + May
	break;
	case 7:
		day += 181;      // Add all of Jan + Feb + March + April + May + June
	break;
	case 8:
		day += 212;      // Add all of Jan + Feb + March + April + May + June + July
	break;
	case 9:
		day += 243;      // Add all of Jan + Feb + March + April + May + June + July + August
	break;
	case 10:
		day += 273;      // Add all of Jan + Feb + March + April + May + June + July + August + Sep
	break;
	case 11:
		day += 304;      // Add all of Jan + Feb + March + April + May + June + July + August + Sep + Oct
	break;
	case 12:
		day += 334;      // Add all of Jan + Feb + March + April + May + June + July + August + Sep + Oct + Nov
	break;
	...
	return EPOCH_2022 + years*86400*365 + day*86400 + hours*3600 + minutes*60 + seconds; // multiply each value by number of seconds in them
}
